...
public class Jesse extends Enemy{
    public Jesse(String id, Point position, List<PImage> images, int actionPeriod, int animationPeriod, Function<Point, Stream<Point>> movement) {
    ...
    public boolean moveTo(WorldModel world, Entity target, EventScheduler scheduler, ImageStore imageStore) {
        ...
        if (this.getPosition().adjacent(target.getPosition())) {
            ...
            if (nextPos.getX()>this.getPosition().getX()){
                this.setImages(imageStore.getImageList("jessRight"));
            }
            else if(nextPos.getX()<this.getPosition().getX()){
                this.setImages(imageStore.getImageList("jessLeft"));
            }
            else if(nextPos.getY()>this.getPosition().getY()){
                this.setImages(imageStore.getImageList("jessDown"));
            }
            else if (nextPos.getY()<this.getPosition().getY()){
                this.setImages(imageStore.getImageList("jessUp"));
            }
    ...
    {
        return new Jesse( id, position, images,
